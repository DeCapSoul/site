...
(function ($) {
    ...
    var $lightboxContainer = $('<div id="lightbox">' + '<div class="lightbox-inner">' + '<div class="controls">' + '<div class="galleryPrev">' + '<span class="ion-ios-arrow-left"></span>' + '</div>' + '<div class="galleryNext">' + '<span class="ion-ios-arrow-right"></span>' + '</div>' + '<div class="galleryClose ion-ios-close-empty fa-2x">' + '</div>' + '</div>' + '</div>' + '<div class="lightbox-text"></div>');


    if ($('.lightbox').length) $('body').append($lightboxContainer);
    var $gallery = [], $galleryCaption = [], $galleryIndex = 0, $lightboxInner = $('body').find('.lightbox-inner');
    ...
    $('.lightbox').each(function () {
        ...
        $this.on('click', function (e) {
            ...
            if (description) {
                $('.lightbox-text').html(description);
            }
            e.preventDefault();
        });
    });
    ...
    $('html').on('click', '.galleryNext, .galleryPrev', function (e) {
        ...
        $('#lightbox [data-index]').css({
            'opacity': 0, 'transform': 'scale(.95)', 'transition': 'opacity .3s, transform .3s'
        });
        $('.galleryClose').css({
            'opacity': 0,
        });
    ...
    function loadLightbox(index, href, caption) {
        var $index = index, $lightboxItem, $lightboxCaption = $('<div class="caption">' + caption + '</div>');
